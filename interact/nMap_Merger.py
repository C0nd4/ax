...
def addHeader(f):
    ...
    nMap_Header += '<?xml-stylesheet href="file://' + expanduser(
        "~") + '/.axiom/interact/includes/nmap-bootstrap.xsl" type="text/xsl"?>'
...
def main_nMapMerger(xmlSet, name):
    ...
    # Regex to match .xml or .xml.~N~ (e.g., .xml, .xml~1~, .xml~24~)
    xml_pattern = re.compile(r'\.xml(\.~\d+~)?$')

    for xml in xmlSet:
        if xml_pattern.search(xml):
...
if __name__ == "__main__":
    ...
    # Regex to match .xml or .xml~N~
    xml_pattern = re.compile(r'\.xml(~\d+~)?$')

    if args.filename is not None:
        f = args.filename
        if xml_pattern.search(f):
        ...
        if os.path.isdir(args.directory):
            ...
            for f in os.listdir(path):
                # Check if the file matches XML pattern
                if xml_pattern.search(f):
